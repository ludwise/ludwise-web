  ...
  for (const unit of units) {
    ...
    if (measureLength && unit.kind !== 'heading') {
      for (const sentence of sentences) {
        const words = countWords(structural.slice(sentence.start, sentence.end));
        if (words > limit)
          report(
            unit,
            sentence.start,
    ...
    for (const term of terminology.terms ?? [])
      for (const synonym of term.prohibitedSynonyms ?? [])
        for (const found of matchesOf(masked, phrasePattern(synonym.term)))
          if (!coveredBy(masked, found.index, found.text.length, synonym.unless ?? []))
            report(
              unit,
              found.index,
              'LW-STE-TERM-PREFERRED',
    ...
    for (const entry of terminology.abbreviations ?? []) {
      if (entry.expandOnFirstUse !== true || expansionReported.has(entry.abbreviation)) continue;
