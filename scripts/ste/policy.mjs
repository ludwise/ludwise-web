...
const validatePolicyFile = (policy) => {
  ...
  if (table[table.length - 1]?.catchAll !== true)
    add('The last classification rule must be the catch-all rule.');
  ...
  for (const limit of ['proceduralSentenceWords', 'descriptiveSentenceWords', 'paragraphSentences'])
  ...
  else if (policy.lexicon.source !== null && !knownLexiconSources().includes(policy.lexicon.source))
    add(
