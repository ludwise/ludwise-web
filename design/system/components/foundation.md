...
Reference specifications for `components/foundation/`. For each component: the **prop contract** (the API you must preserve), the **usage rules**, and the **reference implementation**.
...
Components in this group: `Icon`, `Wordmark`.

---

## Icon

Renders one Lucide glyph inline, inheriting `currentColor` and the LUDWISE 1.75 stroke weight — the only icon primitive in the system.

```jsx
<Icon name="clock" size="sm" />
<Icon name="external-link" size="md" title="Opens the store page in a new tab" />
```

- **Sizes** `xs` 12 (inside chips/badges), `sm` 14 (table cells, inline with body-sm), `md` 16 (default, buttons and labels), `lg` 20 (header actions), `xl` 24 (empty states, section marks).
- **Never** pass `title` when a visible label sits next to the icon — the icon is then decorative and must stay `aria-hidden`.
- **Never** recolour an icon independently of its text. An icon inside a status message inherits that message's text colour.
- Source files live in `assets/icons/`; add a glyph by copying the Lucide SVG there and regenerating this file.
...
export type IconName =
  | "arrow-down" | "arrow-right" | "arrow-up" | "arrow-up-down" | "arrow-up-right"
  | "bell" | "calendar" | "chart-line" | "check" | "chevron-down" | "chevron-left"
  | "chevron-right" | "chevron-up" | "circle-alert" | "circle-check"
  | "circle-question-mark" | "clock" | "clock-fading" | "ellipsis" | "external-link"
  | "eye-off" | "funnel" | "globe" | "grid-2x2" | "heart" | "image-off" | "info"
  | "list" | "list-filter" | "loader-circle" | "menu" | "minus" | "moon" | "plus"
  | "refresh-cw" | "rotate-ccw" | "search" | "settings" | "shield-check"
  | "sliders-horizontal" | "star" | "store" | "sun" | "tag" | "thumbs-up"
  | "triangle-alert" | "user" | "wallet" | "x";
...
export interface IconProps extends Omit<React.SVGProps<SVGSVGElement>, "name"> {
  ...
  /** Normalised to 1.75 across the system. Change only with a documented reason. */
  strokeWidth?: number;
...
export declare const LUDWISE_ICONS: Record<IconName, string>;
export declare const ICON_SIZES: Record<IconSize, number>;
```
...
/* LUDWISE iconography — Lucide (ISC licence, see assets/icons/LICENSE).
   Source SVGs are copied verbatim into assets/icons/. The inner markup is
   inlined here so an icon renders with no network request and inherits
   currentColor. Stroke width is normalised to 1.75 at every size, which reads
   correctly at 14px in a table cell and at 24px in a header. */

export const LUDWISE_ICONS = {
...
export const ICON_SIZES = { xs: 12, sm: 14, md: 16, lg: 20, xl: 24 };

export function Icon({ name, size = "md", strokeWidth = 1.75, title, style, className, ...rest }) {
...
The LUDWISE lockup: the amber step tile beside the split wordmark.

```jsx
<Wordmark size="md" href="/" />          {/* app header */}
<Wordmark size="xl" />                   {/* marketing hero */}
<Wordmark size="sm" mark={false} />      {/* footer, legal */}
<LudwiseMark size={32} />                {/* favicon, avatar, app icon */}
```

**The mark** is a descending three-step path — the same shape `PriceHistoryChart` draws, because LUDWISE plots observations as steps and never as a smooth curve. It also reads as an L. The tile is `--ludwise-amber-400` with a near-black step in **both** themes: near-black on amber clears 9:1 either way, so the mark is one object rather than two designs.

**The wordmark** is Geist Semibold, uppercase, 0.14em tracking, with `WISE` in the accent.

Rules:
- The tile is the only square brand surface. Never place the wordmark itself on an amber field.
- Clear space around the lockup is one tile edge on every side.
- Never restack the lockup vertically, never re-space the letters, never add a tagline inside it.
- `tone="mono"` wherever amber cannot be reproduced. `mark={false}` where the mark is already on screen.
- Minimum sizes: lockup 15px cap height, mark alone **20px** — below that the three treads merge and the staircase stops reading. For a 16px favicon use the mono tile with a two-step path, or the letterform fallback. The step keeps a constant 2.2 stroke at every size and occupies 64% of the tile edge; do not thicken it at small sizes, the treads close up and it stops reading as a staircase.
- Source files: `assets/logo-mark.svg` and `assets/logo-mark-mono.svg`.
...
export interface LudwiseMarkProps {
  /** Tile edge in px. Below 22px the step stroke thickens automatically. */
  size?: number;
...
export interface WordmarkProps {
   ...
   *  surfaces · mono = single colour, for print, favicons and embroidery. */
  tone?: "default" | "inverse" | "mono";
...
/** The tile mark on its own: favicons, app icons, avatars, square slots. */
export declare function LudwiseMark(props: LudwiseMarkProps): React.ReactElement;
...
/* The LUDWISE lockup: an amber tile carrying the step mark, beside the split
   wordmark. The step is the same shape the price-history chart draws — LUDWISE
   plots observations as steps, never as a smooth curve — and it doubles as an L.

   The tile is always amber with a near-black step, in both themes, for the same
   reason the primary button is: near-black on #E8A33D clears 9:1 either way, so
   the mark is one object rather than two designs. tone="mono" drops the amber
   for print, favicons and any context that cannot reproduce it. */

const wordmarkSizeMap = { sm: 15, md: 19, lg: 26, xl: 34 };
const wordmarkTileRatio = 1.22;   /* tile edge relative to cap height */
const wordmarkTileRadius = 0.22;  /* corner radius relative to tile edge */

export function LudwiseMark({ size = 24, tone = "default", style }) {
  ...
  return (
    <span aria-hidden="true" style={{
      ...
      borderRadius: Math.round(size * wordmarkTileRadius),
      background: mono ? "currentColor" : inverse ? "var(--ludwise-neutral-50)" : "var(--ludwise-amber-400)",
      ...style
    }}>
      <svg width={size * 0.64} height={size * 0.64} viewBox="0 0 24 24" fill="none"
        stroke={mono ? "var(--color-background-primary)" : "var(--ludwise-neutral-1000)"}
...
export function Wordmark({ size = "md", tone = "default", mark = true, href, style }) {
  const px = wordmarkSizeMap[size] || wordmarkSizeMap.md;
  const mono = tone === "mono";

  const inner = (
    <span style={{ display: "inline-flex", alignItems: "center", gap: Math.round(px * 0.42) }}>
      {mark && <LudwiseMark size={Math.round(px * wordmarkTileRatio)} tone={tone} />}
      <span style={{
        font: "var(--font-weight-semibold) " + px + "px/1 var(--font-family-display)",
        letterSpacing: "0.14em", textTransform: "uppercase", whiteSpace: "nowrap",
        color: tone === "inverse" ? "var(--color-text-inverse)" : "var(--color-text-primary)",
        ...style
      }}>
        LUD<span style={{ color: mono ? "inherit" : "var(--color-accent-primary)" }}>WISE</span>
      </span>
    </span>
  );

  return href
    ? <a href={href} aria-label="LUDWISE — home" style={{ textDecoration: "none", display: "inline-flex" }}>{inner}</a>
    : <span aria-label="LUDWISE" style={{ display: "inline-flex" }}>{inner}</span>;
}
```
