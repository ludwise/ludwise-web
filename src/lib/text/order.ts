 ...
 * different locales. `/sales` renders two lists derived from the same data:
 ...
 * some visitors and not others. That is a bug nobody can reproduce.
 *
 * The backend orders the data it sends with the identical rule. This function
 * exists because the page adds one entry to that list. The added entry is the
 * pair the visitor asked for, when it holds no sale. The page has to slot it in
 * without breaking the order the backend established.
 */
